...
# Verkort pad naar iets als ".../proj/app/backend" of "~/.../proj/app/backend"
abbr_path() {
  local path="${1:-.}"
  local max=${AGNOSTER_DIR_TRUNC:-3}
  ...
  if (( n <= max )); then
    ...
    local -a tail_parts=(${parts[-$max,-1]})
    if [[ $prefix == "~" ]]; then
      echo "${prefix}/.../${(j:/:)tail_parts}"
    elif [[ $prefix == "/" ]]; then
      echo "/.../${(j:/:)tail_parts}"
    else
      echo ".../${(j:/:)tail_parts}"
...
prompt_git_relative() {
  ...
  if [[ $path_in_repo != '' ]]; then
    prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$(abbr_path "$path_in_repo")"
  fi;
}
...
prompt_dir() {
  if [[ $AGNOSTER_GIT_INLINE == 'true' ]] && $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$(abbr_path "$(git_toplevel | sed "s:^$HOME:~:")")"
  else
    prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$(abbr_path "${PWD/#$HOME/~}")"
  fi
}
